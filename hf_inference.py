...
def load_best_model(sender, best_model_path):

    sender.unpatch_model()

    trained_wts = torch.load(best_model_path)
    updated_wts = sender.state_dict().copy()
    
    for k in list(sender.state_dict().keys()):
        if 'sender.' + k in trained_wts:
            updated_wts[k] = trained_wts['sender.' + k]
        elif k in trained_wts:
            updated_wts[k] = trained_wts[k]

    sender.load_state_dict(updated_wts)
    sender.patch_model(batch_size = 32, prefix_len =10)
...
def main():
    ...
    load_best_model(sender,"./best.pt")
